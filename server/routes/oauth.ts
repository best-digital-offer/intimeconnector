...
oauthRouter.post('/oauth/register', async (req,res) => {
  ...
  if (!body.client_name || redirectUris.length === 0) return res.status(400).json({error:'invalid_client_metadata'});
...
oauthRouter.post('/oauth/token', async (req,res) => {
  ...
  await getSupabaseAdmin().from('oauth_authorization_codes').update({used_at:new Date().toISOString()}).eq('id',record.id);
